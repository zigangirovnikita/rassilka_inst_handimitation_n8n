...
Локальное приложение для ручного n8n-сценария рассылки в Instagram. Приложение хранит отдельные Chrome-сессии Instagram, отправляет Direct до дневного лимита, затем комментирует публикации тех же получателей и возвращает статусы в n8n.
...
- автоматический переход от Direct к комментариям после достижения дневного лимита;
- общий интервал и рабочее окно для сообщений и комментариев;
- выбор обычной публикации вместо закрепленной, с переходом к закрепленным только когда других публикаций нет;
- локальная защита от повторного комментария после сбоя или перезапуска.
...
Когда число успешно отправленных Direct достигает дневного лимита, приложение сохраняет фазу `comments` и начинает отправлять событие `next_comment` в тот же webhook. n8n должен сразу вернуть адресата и готовый текст:

```json
{
  "job_id": "comment_job_12_username",
  "target_username": "username",
  "comment_text": "Здравствуйте! Я написал вам в директ..."
}
```

`job_id` комментария должен быть стабильным и отличаться от `job_id` Direct. Если заданий больше нет, n8n возвращает `{ "no_task": true }`, после чего приложение завершает весь цикл.

Для комментария приложение сначала использует вторую незакрепленную публикацию. Если незакрепленная публикация одна — использует ее. Закрепленные публикации рассматриваются только когда обычных публикаций нет. Поддерживаются посты и Reels.

После подтвержденной публикации приложение отправляет:
...
При пропуске или ошибке отправляется `comment_failed` с полями `reason` и `error`. Возможные причины: `publication_missing`, `post_unavailable`, `comments_disabled`, `comment_box_missing`, `comment_submit_missing`, `comment_text_empty`, `comment_confirmation_missing`, `comment_uncertain`, `n8n_timeout`.
